...
function drawLobby() {
  ...
  // Outer border background (class room wall color)
  background(35, 30, 25);
  ...
  // Map mouse coordinates to 1200x800 system
  let lx = (mouseX - tx) / s;
  let ly = (mouseY - ty) / s;
...
function drawHowToPlay() {
  ...
  // Background: Blueprint/Chalk-grid dark style
  background(10, 16, 25);
  ...
  // 9. Go Back to Lobby Button (at bottom center, Y: 730 to 775)
  // X range: 500 to 700
  let btnX = 600;
  let btnY = 745;
  let btnW = 200;
  ...
  if (hoverBack) {
    cursor(HAND);
    fill(255, 215, 0, 40); // Golden hover fill
  ...
  noStroke();
  fill(hoverBack ? 255 : 200);
  textAlign(CENTER, CENTER);
  textSize(16);
  textStyle(BOLD);
  text("돌아가기 (Lobby)", btnX, btnY);
  pop();
